...
pub const REWARD_RATE: u64 = 1/3000;  // Rewards per second in 1e18 format   0000000000000000003000
...
pub trait StakingContract {
    #[init]   //iqual a constructor
    fn init(&self) { // self, take a struct inmutable
...
fn stake(&self) {
    ...
    if self.staking_position(&caller).is_empty() {
        ...
        let stake_amount = staking_pos.stake_amount.clone();
        let rewards = ( stake_amount /total_staked) * elapsed_time * REWARD_RATE ;
        staking_pos.last_update_timestamp = current_timestamp;
       
        if self.total_staked().get() == BigUint::zero() {
            self.total_staked().set(payment_amount.clone());
        } else {
